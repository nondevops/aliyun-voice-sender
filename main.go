...
import (
	...
	"github.com/n9e/alidyvms-sender/cron"
...
func aconf() {
	...
	*conf = path.Join(runner.Cwd, "etc", "alidyvms-sender.local.yml")
	if file.IsExist(*conf) {
		return
	}

	*conf = path.Join(runner.Cwd, "etc", "alidyvms-sender.yml")
	if file.IsExist(*conf) {
		return
	}
